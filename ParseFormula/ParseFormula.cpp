...
shared_ptr<Formula> ParseFormula::parseRest() {
  ...
  if (getChar() == '(') {
    ...
    if (modality == "") {
      ...
      vector<int> newModalityList = {1};
      return Distributed::create(newModalityList, 1, rest);
    } else {
      if (rest->getType() == FDistributed) {
        Distributed *restDistributed = dynamic_cast<Distributed *>(rest.get());
      ...
      /*for(int charNum = 0, strBeginning=0; charNum < modality.length(); charNum++){
        //cout << charNum << '\n';
        if(modality.at(charNum) ==',' || modality.at(charNum)=='\0'){
          cout << modality.substr(strBeginning, charNum - strBeginning) << '\n';
          newModalityList.push_back(stoi(modality.substr(strBeginning, charNum - strBeginning)));
          strBeginning = charNum + 1;
        }
      }*/
      return Distributed::create(newModalityList, 1, rest);
    }
    ...
    if (modality == "") {
      ...
      vector<int> newModalityList = {1};
      return NotDistributed::create(newModalityList, 1, rest);
    } else {
      if (rest->getType() == FNotDistributed) {
        NotDistributed *restNotDistributed = dynamic_cast<NotDistributed *>(rest.get());
